...
func main() {
	...
	if _, err := os.Stat(templatesDir); os.IsNotExist(err) {
		fmt.Printf("错误: 模板目录不存在: %s", templatesDir)
		os.Exit(1)
	}
	...
	r.GET("/test-page", func(c *gin.Context) {
		c.HTML(http.StatusOK, "test_page.html", gin.H{
			"title": "测试页面 - Auto-Go Mock Server",
		})
	})

	// 表单页面路由（用于条件分支测试）
	r.GET("/form-page", func(c *gin.Context) {
		c.HTML(http.StatusOK, "form_page.html", gin.H{
			"title": "表单页面 - Auto-Go Mock Server",
		})
	})
	...
	r.GET("/catalog", func(c *gin.Context) {
		c.HTML(http.StatusOK, "catalog.html", gin.H{
			"title": "商品目录 - Auto-Go Mock Server",
		})
	})

	// 控制面板页面路由（用于变量和条件组合测试）
	r.GET("/dashboard", func(c *gin.Context) {
		c.HTML(http.StatusOK, "dashboard.html", gin.H{
			"title": "控制面板 - Auto-Go Mock Server",
		})
	})
	...
	wd, err := os.Getwd()
	if err != nil {
		fmt.Printf("错误: 获取当前工作目录失败: %v", err)
		os.Exit(1)
	}

	fmt.Printf("Mock Server 已启动")
	fmt.Printf("服务器地址: http://localhost:%d", port)
	fmt.Printf("模板目录: %s", filepath.Join(wd, templatesDir))
	fmt.Printf("可用页面:\n")
	...
	fmt.Printf("  - 测试页面: http://localhost:%d/test-page\n", port)
	fmt.Printf("  - 表单页面: http://localhost:%d/form-page\n", port)
	fmt.Printf("  - 商品目录: http://localhost:%d/catalog\n", port)
	fmt.Printf("  - 控制面板: http://localhost:%d/dashboard\n", port)
